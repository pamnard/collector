...
describe("refreshItemIndexAfterWrite (#766)", () => {
  ...
  it("with derived jobs wired, released tags prune on write path without waiting for the job", async () => {
    dataDir = await mkdtemp(join(tmpdir(), "collector-item-write-prune-jobs-"));
    const sql = new MemorySqlAdapter();
    const derivedEnqueued: string[] = [];
    const pruneEnqueued: Array<readonly string[]> = [];
    const ctx = {
      fs,
      index: new SqlVaultIndexStore(sql),
      itemDerivedRefreshJobs: {
        enqueue: async (
          ...
          // Job never runs — prune must still happen on the write path.
          derivedEnqueued.push(itemId);
        },
      },
      tagCatalogPruneJobs: {
        enqueue: async (
          ...
          if (candidateTagIds !== undefined) {
            pruneEnqueued.push(candidateTagIds);
          }
        },
      },
    };
    ...
    derivedEnqueued.length = 0;
    pruneEnqueued.length = 0;
    const raw = [
      "---",
      "title: After",
      "type: note",
    ...
    expect(derivedEnqueued).toEqual([itemId]);
    expect(pruneEnqueued).toEqual([[beta.id]]);
  });
