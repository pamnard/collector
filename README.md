...
**MCP** (`collector-mcp`) and **CLI** (`collector-cli`) dial the host over **HTTP** (`POST /api/rpc` + Bearer) using the same host token as the UI. Pass `--base-url` from the READY line and `--data-dir` (or `--token` / `COLLECTOR_HOST_TOKEN`). Missing host or bad auth fails loudly.
